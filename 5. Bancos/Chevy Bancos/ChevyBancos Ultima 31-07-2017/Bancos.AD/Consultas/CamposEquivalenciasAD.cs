...
namespace Bancos.AD.Consultas
{
    public class CamposEquivalenciasAD
    {
        ...
        {
            String query = "SELECT * FROM tb_BAN_CAMPOS_EQUIVALENCIAS AS CE "
                + " INNER JOIN tb_BAN_TABLAS_EQUIVALENCIAS AS TE ON TE.ID=CE.TABLAS_EQUIVALENCIAS"
                + " WHERE (TE.TIPO_ARCHIVO = '" + tipoProceso + "') AND (TE.ID_CUENTA_BANCO='" + IdCuentaBanco + "')"
